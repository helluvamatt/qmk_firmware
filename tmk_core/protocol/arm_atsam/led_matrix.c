...
void led_matrix_prepare(void)
{
    ...
    for (uint8_t i = 0; i < ISSI3733_LED_COUNT; i++)
    {
        if (led_map[i].x < disp.left) disp.left = led_map[i].x;
        if (led_map[i].x > disp.right) disp.right = led_map[i].x;
        if (led_map[i].y < disp.bottom) disp.bottom = led_map[i].y;
        if (led_map[i].y > disp.top) disp.top = led_map[i].y;
    }
    ...
    for (uint8_t i = 0; i < ISSI3733_LED_COUNT; i++)
    {
        led_map[i].px = (led_map[i].x - disp.left) / disp.width * 100;
        led_map[i].py = (led_map[i].y - disp.top) / disp.height * 100;
        *led_map[i].rgb.r = 0;
        *led_map[i].rgb.g = 0;
        *led_map[i].rgb.b = 0;
    }
